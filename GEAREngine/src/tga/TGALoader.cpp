...
{																			// TGA Loading code nehe.gamedev.net)
	...
	if(texture->bpp == 24)													// If the BPP of the image is 24...
		texture->type	= 1;											// Set Image type to GL_RGB
	else																	// Else if its 32 BPP
		texture->type	= 2;											// Set image type to GL_RGBA
	...
	int nbytesPerRow=0;
	if(texture->type==1)
		nbytesPerRow=tga.Width*3;
	else if(texture->type==2)
		nbytesPerRow=tga.Width*4;

	unsigned char* row_buffer = new unsigned char[nbytesPerRow];

	for(unsigned int y = 0; y < (int)tga.Height/2; y++)
	{
		memcpy(row_buffer, &texture->imageData[y*nbytesPerRow], nbytesPerRow);

		unsigned char* bottom_row_buffer=&texture->imageData[(tga.Height-1-y)*nbytesPerRow];
		memcpy(&texture->imageData[y*nbytesPerRow], bottom_row_buffer, nbytesPerRow);
		memcpy(bottom_row_buffer, row_buffer, nbytesPerRow);
	}
	delete [] row_buffer;
	//
