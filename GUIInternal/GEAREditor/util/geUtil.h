...
public:
    ...
    static void convertPathToUnixFormat(char* path) {
        ...
        for (int x = 0; x < static_cast<int>(strlen(path)); x++) {
            if (path[x] == '\\')
                path[x]='/';
        }
        //
    ...
    static bool isSubString(const char* str, const char* substr) {
        ...
        while (!bFound) {
            int sub_strlen = static_cast<int>(strlen(substr));
            int str_len = static_cast<int>(strlen(fond_str));
...
private:
    static const char* STRCHR_nocase(const char* str, char value) {
        for (int x = 0; x < static_cast<int>(strlen(str)); x++) {
