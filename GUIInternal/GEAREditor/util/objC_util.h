...
//		NSArray *pathArray = [stringFromChar componentsSeparatedByCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"."]];
    ...
    static unsigned int loadTexture(FILE* fp, bool& bAlpha, unsigned int& width, unsigned int& height, unsigned int& bpp)
    {
        ...
        unsigned long gl_ext_algo_compressedSz=0;
        
        fread(&compressedSz, sizeof(unsigned long), 1, fp);        

        if(format==5 || format==6 || format==7 || format==8 || format==9 || format==10)
        {
            fread(&gl_ext_algo_compressedSz, sizeof(unsigned long), 1, fp);
        }
        
        unsigned char* compressed_buffer=(unsigned char*)malloc(compressedSz);
        fread(compressed_buffer, compressedSz, 1, fp);
        
        unsigned long un_com_sz=width*height*bpp;
        if(format<5)
        {
...
//        }
        ...
        switch (format) {
            ...
            case 5:
                glCompressedTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG, width, height, 0, gl_ext_algo_compressedSz, buffer);
                break;
            case 6:
                glCompressedTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG, width, height, 0, gl_ext_algo_compressedSz, buffer);
                break;
#endif

#ifdef GL_AMD_compressed_ATC_texture
            case 7:
                glCompressedTexImage2D(GL_TEXTURE_2D, 0, GL_ATC_RGB_AMD, width, height, 0, gl_ext_algo_compressedSz, buffer);
                break;
            case 8:
                glCompressedTexImage2D(GL_TEXTURE_2D, 0, GL_ATC_RGBA_INTERPOLATED_ALPHA_AMD, width, height, 0, gl_ext_algo_compressedSz, buffer);
            ...
            case 9:
                glCompressedTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGB_S3TC_DXT1_EXT, width, height, 0, gl_ext_algo_compressedSz, buffer);
                break;
            case 10:
                glCompressedTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, width, height, 0, gl_ext_algo_compressedSz, buffer);
                break;
#endif
    ...
    static unsigned int loadTexture2(const char* filepath, bool& bAlpha, unsigned int& width, unsigned int& height, unsigned int& bpp)
    {
        //replace .png to .tzx
...
#if 0
    static unsigned int loadTexture2(stFD* fd, bool& bAlpha, unsigned int& width, unsigned int& height, unsigned int& bpp)
    {
    	if(fd==NULL)
    	{
